    ...
    <div className="page home-page">
      ...
      <section className="section">
        ...
        <div className="highlight-grid">
          <div className="card"><div className="highlight-title"><span aria-hidden="true">🔥</span><div><h3>최근 자주 나온 번호</h3><p className="body-small">최근 10회 출현 횟수</p></div></div><div className="highlight-numbers">{hot.map((stat) => <div key={stat.number}><LottoBall number={stat.number} /><strong>{stat.countRecent10}회</strong></div>)}</div></div>
          <div className="card"><div className="highlight-title"><span aria-hidden="true">🧊</span><div><h3>오래 쉬고 있는 번호</h3><p className="body-small">최신 회차 기준</p></div></div><div className="highlight-numbers">{cold.map((stat) => <div key={stat.number}><LottoBall number={stat.number} /><strong>{stat.gap}회</strong></div>)}</div></div>
        </div>
      </section>
    </div>
  );
