...
const presets = ["hot", "cold", "fixed", "carryover", "pair", "birthday"] as const;
...
export async function generateMetadata({ params }: { params: Promise<{ preset: string }> }): Promise<Metadata> {
  ...
  const titles: Record<string, string> = { hot: "핫넘버 추첨", cold: "미출현 번호 추첨", fixed: "내 번호 넣기", carryover: "이월수 추첨", pair: "궁합수 추첨", birthday: "기념일 번호 추첨" };
